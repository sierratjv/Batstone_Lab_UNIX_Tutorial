...
), ksh (KornShell), bash (Bourne Again Shell), zsh (Z shell). When a user typies a command, the shell searches the filestore for a program to run that command and requests the kernel to execute the program (Reference: UNIX Tutorial for Beginners. http://www.ee.surrey.ac.uk/Teaching/Unix/unixintro.html). 
